(function(){
  ...
  function polishMobileNav(d){
    ...
    var sign=signed.querySelector('.signout');if(sign)sign.textContent='Sign out';
    if(window.innerWidth<=700&&!header.querySelector('.v3-mobile-user-photo')){
      var photo=signed.querySelector('.nav-user-photo,.nav-user-initials');
      ...
      if(sign)nav.appendChild(sign);
      signed.style.display='none';
    }
    if(!d.getElementById('v3MobileNavStyle')){var s=d.createElement('style');s.id='v3MobileNavStyle';s.textContent='@media(max-width:700px){header{position:relative}.v3-mobile-user-photo{position:absolute;right:16px;top:12px;display:flex}.v3-mobile-user-photo .nav-user-photo,.v3-mobile-user-photo .nav-user-initials{width:34px;height:34px}.nav{display:flex!important;flex-wrap:nowrap!important;gap:0!important;width:100%!important;align-items:center}.nav button{flex:1 1 auto!important;min-width:0!important;padding:9px 3px!important;font-size:12px!important;white-space:nowrap!important}.nav .signout{color:#9b2525!important;flex:1 1 auto!important;width:auto!important;font-size:12px!important;padding:9px 3px!important}.top{gap:8px!important}}';d.head.appendChild(s);}
  }
  ...
  function polishAdminLayout(d){
    ...
    if(!d.getElementById('v3AdminLayoutStyle')){var st=d.createElement('style');st.id='v3AdminLayoutStyle';st.textContent='.v3-admin-head{display:flex!important;flex-direction:column!important;align-items:stretch!important;gap:10px!important}.v3-admin-description{white-space:nowrap!important;margin:0!important}.v3-admin-head-actions{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;gap:8px!important;margin-top:0!important}.v3-admin-head-actions .btn{flex:1 1 0!important;white-space:nowrap!important}@media(max-width:700px){.v3-admin-description{font-size:15px!important}.v3-admin-head-actions .btn{padding:10px 6px!important;font-size:11px!important}}';d.head.appendChild(st);}
  }
  ...
  function installPrintExit(d){
    var b=d&&d.getElementById('v3Print');if(!b||b.dataset.v3PrintExitFixed)return;b.dataset.v3PrintExitFixed='1';
    b.onclick=function(){
      ...
      var w=win.open('','_blank','width=1200,height=800');if(!w){alert('Please allow pop-ups for this site to print the list.');return;}
      w.document.write('<!doctype html><html><head><meta charset="utf-8"><title>COEP ENTC 1983 — Class Reunion</title><style>@page{size:landscape;margin:12mm}body{font-family:Arial;color:#222;margin:0}.toolbar{display:flex;gap:10px;margin:0 0 16px;padding:12px;border-bottom:1px solid #ddd;position:sticky;top:0;background:#fff}button{padding:9px 15px;border:1px solid #cdbb9e;border-radius:10px;background:#fff;font-weight:700;cursor:pointer}button.print{background:#c98616;color:#fff;border-color:#c98616}@media print{.toolbar{display:none}}h1{font:700 22px Georgia;margin:0 0 5px}p{margin:0 0 14px;color:#666;font-size:12px}table{width:100%;border-collapse:collapse;font-size:11px}th,td{border:1px solid #aaa;padding:6px 7px;text-align:left;vertical-align:top}th{font-weight:700}</style></head><body><div class="toolbar"><button onclick="window.close()">← Exit</button><button class="print" onclick="window.print()">Print</button></div><h1>COEP ENTC 1983 — Class Reunion</h1><p>Class list</p><table><thead><tr><th>S.N.</th><th>Name</th><th>Address</th><th>Town / City</th><th>Phone No.</th><th>Email</th></tr></thead><tbody>'+rows+'</tbody></table></body></html>');w.document.close();
    };
  }
  function install(){var d=target();cleanPhotoAdmin();polishMobileNav(d);polishPhotoInput(d);polishAdminLayout(d);polishSharePrint(d);installPrintExit(d);var f=document.getElementById('app');try{if(f&&f.contentDocument&&!f.contentDocument.__v3UiObserver){var obs=new MutationObserver(function(){cleanPhotoAdmin();polishMobileNav(f.contentDocument);polishPhotoInput(f.contentDocument);polishAdminLayout(f.contentDocument);polishSharePrint(f.contentDocument);installPrintExit(f.contentDocument);});obs.observe(f.contentDocument.body,{childList:true,subtree:true});f.contentDocument.__v3UiObserver=true;}}catch(e){}}
