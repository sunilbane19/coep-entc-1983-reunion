(function(){
  ...
  function labels(){var ms=window.adminMembers||[],c=document.getElementById('filterComplete'),p=document.getElementById('filterPending');if(c)c.textContent='✓ Done '+ms.filter(function(m){return !!m.profile_completed;}).length;if(p)p.textContent='○ Not Done '+ms.filter(function(m){return !m.profile_completed;}).length;}
  ...
  // Keep the original page lifecycle intact. The page's own render() calls
  // the lexical photosPage()/adminPage() functions. We only provide the
  // album renderer for the hashchange path and add the non-rendering tools.
  ...
  window.addEventListener('hashchange',function(){
    if(location.hash==='#photos'){
      ...
      setTimeout(function(){photosPageV3().finally(function(){var a=document.getElementById('app');if(a)a.style.visibility='visible';window.scrollTo(0,0);});},0);
    }
    if(location.hash==='#admin')setTimeout(addAdminTools,50);
  });

  setTimeout(function(){
    if(location.hash==='#photos')photosPageV3().finally(function(){var a=document.getElementById('app');if(a)a.style.visibility='visible';});
